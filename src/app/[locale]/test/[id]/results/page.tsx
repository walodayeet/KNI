...
export default function TestResultsPage() {
  ...
  const isDailyTest = searchParams.get('daily') === 'true';
  ...
  const fetchResults = async () => {
    try {
      ...
      // Fetch user streak if this is a daily test
      if (isDailyTest) {
        try {
          const streakResponse = await fetch('/api/daily-mock-test');
          if (streakResponse.ok) {
            const streakData = await streakResponse.json();
            setUserStreak(streakData.user?.daily_streak || 0);
          }
        } catch (streakError) {
                ...
                <CardContent>
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600">
                      {userStreak === 1 ? 'day' : 'days'} in a row
                    </p>
                    {userStreak > 0 && (
