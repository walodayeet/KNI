...
const startAttemptSchema = z.object({
  ...
  isDailyTest: z.boolean().optional().default(false)
});
...
export async function POST(
  ...
  try {
    ...
    const { userId, isDailyTest } = startAttemptSchema.parse(body);
...
export async function PUT(
  ...
  try {
    const body = await request.json();
    const { attemptId, answers, timeTaken, isDailyTest } = submitAttemptSchema.parse(body);
