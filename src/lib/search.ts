...
export class SearchEngine extends EventEmitter {
  ...
  async addDocument(
    indexId: string,
    document: Omit<SearchDocument, 'createdAt' | 'updatedAt'>
  ): Promise<void> {
  ...
  async search(indexId: string, searchQuery: SearchQuery): Promise<SearchResult> {
    ...
    // Generate facets
    const facets = searchQuery.facets
      ? this.generateFacets(documents, searchQuery.facets)
      : undefined

    // Generate suggestions
    const suggestions = this.config.enableSuggestions
      ? this.generateSuggestions(searchQuery.query, index)
      : undefined
  ...
  private generateSuggestions(query: string, index: SearchIndex): string[] {
    ...
    for (const doc of index.documents) {
      ...
      const allTokens = titleTokens.concat(contentTokens)
